...
class Web extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($inputCaptcha === $sessCaptcha) {
                ...
                $this->session->set_flashdata('_msg_q', 'Please type the correct code!'); // Captcha verification is incorrect.  
                redirect('web');
            }
        }
    ...
    function deleteCaptchaContent() {
        $this->load->helper('directory');
        $this->load->helper('file');

        $dir_fiels = directory_map('captcha_images/');
        $len = sizeOf($dir_fiels);
        for ($i = 0; $i < $len; $i++) {
            unlink('captcha_images/' . $dir_fiels[$i]);
        }
    }
